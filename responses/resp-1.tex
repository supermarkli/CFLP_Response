...
We have clarified this in the revised Section IV-B:
\begin{changes}
The enclave memory pool is reserved at boot time, but memory allocation is dynamic. \textcolor{blue}{When enclaves are created, they request memory from the pool through the Memory Manager, which updates the ownership table. When enclaves terminate, their memory is returned to the pool. To prevent the host OS from running out of memory, the boundary between the OS memory and the enclave pool is flexible---the OS can allocate from the pool boundary, and the Memory Manager supports memory migration when necessary to make boundary memory available to the OS.}
\end{changes}
\end{revresponse}
...
\begin{itemize}
\item \textbf{FIFO (First-In-First-Out):} Simpler to implement but showed $\sim$12\% more LPMP faults in memory-intensive workloads
\item \textbf{Random replacement:} Even simpler but increased LPMP faults by $\sim$15\%
...
\textbf{COFFER's TLB Security Mechanisms:} As described in Section III-B, COFFER enforces two security principles for TLB management:
...
\item \textbf{ASID Support (When Available):} On RISC-V processors supporting ASIDs (Address Space IDs), COFFER can leverage them for additional TLB isolation between address spaces.
\end{enumerate}
...
\item ASID support is not universal in RISC-V implementations
\item Sophisticated attacks exploiting shared TLB resources or ASID-oblivious TLB entries may still be possible
